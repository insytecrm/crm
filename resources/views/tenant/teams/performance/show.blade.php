<x-tenant-layout :title="$team->name . ' | ' . __('Performance') . ' | InSyte CRM'">
    <div x-data="{ filtersOpen: @js($periodFilter->isReportsFiltered()) }">
        <div class="mb-3">
            <a
                href="{{ route('tenant.teams.performance.index', $periodFilter->toReportsQueryArray()) }}"
                class="inline-flex items-center gap-1 text-sm font-medium text-slate-500 transition hover:text-black"
            >
        ...
        <x-tenant.list-toolbar>
            ...
            <x-slot:panel>
                @include('tenant.reports.partials.filter-panel', [
                    ...
                    'action' => route('tenant.teams.performance.show', $team),
                    'clearHref' => route('tenant.teams.performance.show', $team),
                ])
            </x-slot:panel>
        </x-tenant.list-toolbar>
        ...
        @if ($cards === [])
            <div class="mt-3 rounded-xl border border-slate-100 bg-white px-4 py-12 text-center shadow-sm">
                <p class="text-sm text-slate-500">{{ __('No members yet.') }}</p>
            </div>
        @else
