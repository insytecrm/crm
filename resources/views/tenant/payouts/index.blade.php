<x-tenant-layout :title="__('Payouts') . ' | InSyte CRM'">
    ...
    >
        ...
        <div class="overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-sm">
            <div class="border-b border-slate-100 px-4 py-3 sm:px-5">
                <h2 class="text-sm font-semibold text-black">{{ __('All Payouts') }}</h2>
            </div>
            ...
            <x-tenant.manageable-table.bulk-bar
                ...
                :confirm-message="__('Are you sure you want to delete the selected payouts?')"
            />
                ...
                <table class="min-w-full divide-y divide-slate-100">
                    <thead class="bg-slate-50">
                        <tr class="align-middle">
                            ...
                            <th x-show="isColumnVisible('payout_status')" class="whitespace-nowrap px-4 py-3 text-start align-middle text-xs font-semibold uppercase tracking-wider text-slate-500">{{ __('Payout Status') }}</th>
                    ...
                    <tbody class="divide-y divide-slate-100 bg-white">
                        @forelse ($payouts as $payout)
                            @include('tenant.payouts.partials.payout-row', ['payout' => $payout])
                        @empty
                            <tr>
                                <td colspan="20" class="px-4 py-12 text-center text-sm text-slate-500">
                                    {{ __('No payouts yet. Mark agreement on a booking to record payout details.') }}
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
    ...
    @push('modals')
        ...
        @foreach ($payouts as $payout)
            @include('tenant.payouts.partials.payout-details-modal', ['payout' => $payout])
        @endforeach
    @endpush
</x-tenant-layout>
