...
>
    ...
    <div
        ...
        style="display: none;"
    >
        <p class="text-xs font-semibold text-black">{{ $heading }}</p>
        <p class="mt-0.5 text-[11px] text-slate-500">{{ __('Add completion notes (optional)') }}</p>
        ...
        <form method="POST" action="{{ $action }}" class="mt-2.5 space-y-2.5">
            ...
            <div class="flex items-center justify-end gap-2">
                ...
                <button
                    type="submit"
                    class="rounded-lg bg-navy px-3 py-1.5 text-xs font-semibold text-white shadow-sm transition hover:bg-navy/90"
                >
                    {{ __('Mark Complete') }}
                </button>
            </div>
